...
1. We didn't find strong correlation between the variables (see Figure \ref{fig:CorrPlot} in Appendix).
...
After conducting our initial exploratory data analysis (EDA), we identified necessary transformations and feature engineering to prepare our data for analysis. 

We removed a few bookings with negative adr values or values exceeding 5400, which appeared incorrect, and imputed missing values in the children column with 0. The features lead\_time, days\_in\_waiting\_list and adr had a skewed distribution. So we log transformed them to ensure their distribution is almost normal in nature.  arrival\_date\_month was modified into the column arrival\_month by substituting the month names with the corresponding month number. 

A few new columns were created by combining groups within categorical columns or by combining two or more columns. The column domestic was created by assigning the value 'domestic' if the country of origin was Portugal and 'international' if not. We defined the binary feature got\_room\_booked  by assigning values 1 or 0 depending on whether reserved\_room\_type is equal to assigned\_room\_type or not. Similarly, a new feature required\_car\_parking was assigned values 1 or 0 depending on whether the required\_car\_parking\_spaces were more than zero or not. The features booked\_by\_agent and booked\_by\_company were created conditioned on whether the columns agent or company were missing. total\_nights column was created by summing of stays\_in\_weekend\_nights and stays\_in\_week\_nights. Similarly, previous\_bookings was created by summing previous\_cancellations and previous\_bookings\_not\_cancelled. 

Since the data set contained ISO3166 codes to represent countries, we used a mapping file to map the codes to a country and to a continent.\footnote{Mapping file available at  https://github.com/lukes/ISO-3166-Countries-with-Regional-Codes/blob/master/all/all.csv} We only used the continent as a categorical variable for our analysis as countries would have too many factors.
